...
{
	unsigned int idxOffset = (threadIdx.x + blockIdx.x * blockDim.x);
	
	while (idxOffset < length) {
		a2[idxOffset] = a[idxOffset] * a[idxOffset];
		b2[idxOffset] = b[idxOffset] * b[idxOffset];
		idxOffset += gridDim.x * blockDim.x;
	}
}
